...
# Top nav
st.markdown("""<nav style="position:sticky; top:0; background:white; display:flex; align-items:center; padding:1rem 2rem; box-shadow:0 2px 8px rgba(0,0,0,0.1); z-index:1000;">
...
button.primary{background:#FFADC6; color:white; border:none; border-radius:6px; padding:0.8em 1.5em; font-size:1rem; cursor:pointer;}
...
st.markdown("""<h1>AI Lung Diagnostics, Reimagined</h1>
<p>A premium prototype to assist clinicians & patients.</p>
<button class="primary" onclick="window.location.href='#diagnosis'">Start Diagnosis 💡</button>""", unsafe_allow_html=True)
...
with st.form("form"):
    ...
    ok=st.form_submit_button("Analyze X‑ray")
    st.markdown('</div>', unsafe_allow_html=True)
if ok:
    if not uploaded:
        st.error("Upload an X‑ray.")
        st.stop()
    img=Image.open(uploaded).convert("RGB")
    img_r=img.resize((512,512))
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.image(img_r,caption="Original",use_column_width=True)
    ...
    hrgb=np.zeros((*heat.shape,3),dtype=np.uint8); hrgb[:,:,0]=heat
    ov=np.clip(np.array(img_r)*0.6+hrgb*0.4,0,255).astype(np.uint8)
    st.image(ov,caption="Heatmap",use_column_width=True)
    st.success("**Diagnosis:** Pneumonia Detected")
    st.markdown("**Explanation:** Regions show consolidation.")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div id="report" style="padding:4rem 2rem;">', unsafe_allow_html=True)
    st.header("Report Preview")
    st.markdown(f'<div class="card"><b>Name:</b> {name}<br><b>Age:</b> {age}<br><b>Gender:</b> {gender}<br><b>Symptoms:</b> {symptoms or "N/A"}<br><b>Activity:</b> {activity}<br><b>Exposure:</b> {exposure or "N/A"}<br><b>Smoking:</b> {smoking}<br><b>HRV (ms):</b> {hrv or "N/A"}</div>', unsafe_allow_html=True)
    st.download_button("📄 Download Report",data=generate_docx(name,age,gender,symptoms,activity,exposure,smoking,hrv),file_name=f"Report_{name.replace(' ','_')}.docx",mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    st.markdown('</div>',unsafe_allow_html=True)

# About
st.markdown('<div id="about" style="padding:4rem 2rem;">', unsafe_allow_html=True)
st.header("About AMAL")
st.write("Prototype AI lung assistant with premium white & pink design.")
st.markdown('</div>', unsafe_allow_html=True)
